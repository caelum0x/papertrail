// Client-side fetch helper for the science pages. Reads the active org id from
// localStorage (set by the console layout's org switcher) and forwards it as the
// x-org-id header so withOrg scopes the request to the right tenant. No server
...
export async function scienceGet<T>(path: string): Promise<ApiEnvelope<T>> {
...
export async function scienceSend<T>(
  path: string,
