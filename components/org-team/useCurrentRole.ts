...
// Resolves the current user's role in the active org (from the session
// endpoint) so pages can gate management actions to owner/admin. Falls back to
// null while loading or if the org can't be resolved.
...
export function useCurrentRole(): CurrentRoleState {
  ...
  const canManage = role ? (RANK[role] ?? 0) >= RANK.admin : false;
  return { role, loading, canManage };
}
