...
import type { ErrorLevel, HealthStatus } from "@/lib/observability/types";

// Small shared presentational primitives used across the observability views:
// status/level pills, loading/empty/error states, and timestamp formatting.
...
export function HealthPill({ status }: { status: HealthStatus }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${HEALTH_STYLES[status]}`}
    >
...
const LEVEL_STYLES: Record<ErrorLevel, string> = {
  debug: "bg-paper text-ink/50 border-ink/10",
...
export function LevelBadge({ level }: { level: ErrorLevel }) {
  return (
    <span
      className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[11px] font-medium uppercase tracking-wide ${LEVEL_STYLES[level]}`}
    >
      {level}
    </span>
  );
