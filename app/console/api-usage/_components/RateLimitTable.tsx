import type { RateLimitEventItem } from "@/lib/apiusage/types";
import { formatDateTime, keyLabel } from "./shared";

// The rate-limit events table body. Empty/loading/error states are handled by the
// caller via TableStates so this only renders when there are rows.
export function RateLimitTable({ items }: { items: RateLimitEventItem[] }) {
  return (
    <table className="w-full text-left text-sm">
      <thead>
        <tr className="border-b border-ink/10 text-xs uppercase tracking-wide text-ink/40">
          <th className="px-4 py-2 font-medium">Time</th>
          <th className="px-4 py-2 font-medium">Route</th>
          <th className="px-4 py-2 font-medium">Key</th>
        </tr>
      </thead>
      <tbody className="divide-y divide-ink/10">
        {items.map((item) => (
          <tr key={item.id} className="hover:bg-paper/60">
            <td className="px-4 py-2 text-ink/40">
              {formatDateTime(item.createdAt)}
            </td>
            <td className="px-4 py-2">
              <code className="text-ink/80">{item.route}</code>
            </td>
            <td className="px-4 py-2 text-ink/60">
              {keyLabel(item.keyName, item.apiKeyId)}
            </td>
          </tr>
        ))}
      </tbody>
    </table>
  );
}
